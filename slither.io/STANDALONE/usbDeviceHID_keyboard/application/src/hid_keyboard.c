...
static void Keyboard_UpdateReport(void)
{
	...
	{
		/* 'o' */
		HID_KEYBOARD_REPORT_SET_KEY_PRESS(g_keyBoard.report, 0x12);
	}
	else if(!ciaaReadInput(1))
	{
		/* 'p' */
		HID_KEYBOARD_REPORT_SET_KEY_PRESS(g_keyBoard.report, 0x13);
	}
	else if(!ciaaReadInput(2))
	{
		/* 'q' */
		HID_KEYBOARD_REPORT_SET_KEY_PRESS(g_keyBoard.report, 0x14);
	}
	else if(!ciaaReadInput(3))
	{
		/* 'r' */
...
static ErrorCode_t Keyboard_SetReport(USBD_HANDLE_T hHid, USB_SETUP_PACKET *pSetup, uint8_t * *pBuffer, uint16_t length)
{
	...
	case HID_REPORT_OUTPUT:
		 ...
		 */
		if (**pBuffer & 0x01) {
			Board_LED_Set(LEDS_LED2, 1);
		}
		else {
			Board_LED_Set(LEDS_LED2, 0);
		}
		break;
